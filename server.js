...
const path = require('path'); // Added path module for serving static files
...
app.post("/hospitals/signup", async (req, res) => {
    ...
    try {
        ...
        if (existingHosp) {
            res.send(`<script>alert("Hospital ID already exists. Please choose a different hospital ID."); window.location.href = "/hospitals/signup";</script>`);
        } else {
            const hashedPassword = await bcrypt.hash(password, 10);
